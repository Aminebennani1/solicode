...
<body>
    ...
    <?php
        try{
            ...
            $stm = $db->prepare("SELECT COUNT(*) AS total FROM `book`");
            $stm->execute();
            $res = $stm->fetch();
            $total = $res["total"];
    ...
    <table>
        ...
        <tbody id="data">
            <?php
                ...
                try{
                    include_once "connection.php";
                    $stm = $db->prepare("SELECT  C.client_id, T.tour_id, B.book_date, C.client_fullname, C.client_email, C.client_phone,  T.tour_name 
